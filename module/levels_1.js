...
import {P as A} from "./plugin-base.js";
import {p as E} from "./positions.js";
const C = {
    interval: 60 * 60 * 24,
    clearTimeout: 3e3
}
    ...
    draw(t) {
        ...
        t.useMediaCoordinateSpace(s=>{
            ...
            this._data.forEach(r=>{
                const c = E(r.priceY, i, e.lineWidth)
                  , l = (c.position + c.length / 2) / i;
                e.fillStyle = r.color,
                e.strokeStyle = r.color,
                e.lineDashOffset = 0,
                e.globalAlpha = r.fade ? .5 : 1,
                e.beginPath(),
                e.moveTo(r.startX + 4, l),
                e.lineTo(r.endX - 4, l),
                e.stroke(),
                e.beginPath(),
                e.setLineDash([3, 6]),
                e.lineCap = "round",
                e.moveTo(r.startX - 30, l),
                e.lineTo(s.mediaSize.width, l),
                e.stroke(),
                e.setLineDash([]),
                e.beginPath(),
                e.arc(r.startX, l, 4, 0, 2 * Math.PI),
                e.arc(r.endX, l, 4, 0, 2 * Math.PI),
                e.fill(),
                e.font = "10px sans-serif";
                const h = e.measureText(r.text);
                e.beginPath(),
                e.roundRect(r.startX - 30 - 20 - h.width, l - 7, h.width + 20, 14, 4),
                e.fill(),
                e.fillStyle = "#FFFFFF",
                e.fillText(r.text, r.startX - 30 - 15 - h.width, l + 3),
                e.save(),
                e.translate(r.startX - 30 - 14, l - 6);
                const d = 12 / L;
                e.scale(d, d),
                e.fill(r.icon, "evenodd"),
                e.restore()
            }
            )
        }
...
const k = S("chart", {
  ...
  , _ = k.addLineSeries({
    lastPriceAnimation: P.OnDataUpdate
})
  , g = X()
  , [F,n] = [g.slice(0, 400), g.slice(400)];
_.setData(F);
const u = new y(_,{
    interval: 60
})
  , v = a=>{
    a === n[0].time && u.addExpiringAlert(19.5, n[0].time, n[25].time, {
        crossingDirection: "down",
        title: "$19.50"
    }),
    a === n[30].time && u.addExpiringAlert(19.75, n[30].time, n[45].time, {
        crossingDirection: "up",
        title: "$19.75"
    }),
    a === n[45].time && u.addExpiringAlert(19, n[45].time, n[65].time, {
        crossingDirection: "down",
        title: "$19.00"
    }),
    a === n[55].time && u.addExpiringAlert(20, n[55].time, n[65].time, {
        crossingDirection: "up",
        title: "$20.00"
    }),
    a === n[75].time && u.addExpiringAlert(21.25, n[80].time, n[220].time, {
        crossingDirection: "up",
        title: "wishful"
    })
}
