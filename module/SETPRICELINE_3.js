    ...
    draw(t) {
        if(this._data.visible ){
            t.useBitmapCoordinateSpace(i=>{
                const ctx = i.context
                // const o = g(this._data.rightX - marginOffset + 1, this._data.rightX - 1, i.horizontalPixelRatio)
                const calcW = g(this._data.leftX + marginOffset + 1, this._data.leftX + 1, i.horizontalPixelRatio)
                const calcH = L(this._data.y, i.verticalPixelRatio, marginOffset);
                ctx.fillStyle = "crimson";//this._data.color;
                const l = [3, 6, 6, 3].map(p=>p * i.horizontalPixelRatio);
                // const l = [5, 0, 0, 5].map(p=>p * i.horizontalPixelRatio);
                ctx.beginPath();
                ctx.roundRect(calcW.position, calcH.position, calcW.length, calcH.length, l);
                ctx.fill();
            ...
            // }
                ctx.translate(calcW.position + 3 * i.horizontalPixelRatio, calcH.position + 3 * i.verticalPixelRatio),
