...
class PointDraw_Class {
    ...
    draw(parent) {
        parent.useBitmapCoordinateSpace(i=>{
            ...
            // ctx.setLineDash([6, 8])
            ctx.lineCap = "round";
            ctx.moveTo(p1x, p1y);        //DRAW-PLUS
            ctx.lineTo(p1x-10, p1y);
            ctx.moveTo(p1x-5, p1y+10);        //DRAW-PLUS
            ctx.lineTo(p1x-5, p1y-10);
            ctx.moveTo(p2x+10, p2y+10);        //DRAW-X
            ctx.lineTo(p2x-10, p2y-10);
            ctx.moveTo(p2x+10, p2y-10);
            ctx.lineTo(p2x-10, p2y+10);
            // ctx.lineTo(p2x, p2y);
            ctx.stroke();
